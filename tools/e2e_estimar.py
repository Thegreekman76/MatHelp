#!/usr/bin/env python3
# tools/e2e_estimar.py — E2E del juego "Estimación" (redondeo + "¿está cerca?", F9).
#
# Escenario (contra `fitz run` o el binario):
#   1. Perfil -> GET /juegos muestra "Estimación" (href="/estimar").
#   2. GET /estimar -> el SSR trae la cuenta (.est-eq) con data-*.
#   3. Abre WS /live/estimar; en cada turno LEE kind/a/op/b, RECOMPUTA la respuesta
#      (redondeo / suma / producto / porcentaje) y la tipea. El valor exacto siempre
#      cae dentro de la tolerancia (tol=0 en redondeo → exacto).
#   4. Verifica: data-fb-seq, q-racha, q-progress, estrellas, ronda perfecta +
#      confetti + récord; y en Postgres: sesión mode='estimar' topic 'estimar', 10
#      attempts TODOS correctos, best_streak=10 en profile_game_stats.
#
# Requiere: requests, websocket-client, psql en PATH (o FITZ_PSQL).
# Uso:  python tools/e2e_estimar.py --only build   (o --only run, o ambos)
...
def resolver(html):
    m = re.search(r'est-eq[^"]*" data-kind="(\w+)" data-a="(\d+)" data-op="([^"]+)" data-b="(\d+)"', html)
    assert m, "no llegó la cuenta (.est-eq con data-*)"
    a, op, b = int(m.group(2)), m.group(3), int(m.group(4))
    if op == "R":  # redondeo a la base b
        return ((a + b // 2) // b) * b
    if op == "+":
        return a + b
    if op == "×":
    ...
    if op == "%":
        return a * b // 100
    raise AssertionError(f"operador desconocido: {op!r}")
...
def open_ws(s):
    ...
    ws = websocket.create_connection(BASE.replace("http", "ws") + "/live/estimar", header=[f"Cookie: {cookies}"], timeout=10)
...
def answer(ws, cid, ei, valor):
    ws.send(json.dumps({"event": "digito", "payload": {"d": "1", "component_name": "estimar", "instance_id": cid}, "html": "", "patches": []}))
    ws.recv()
    ws.send(json.dumps({"event": "answer", "payload": {"v": str(valor), "ei": str(ei), "touched": "1", "component_name": "estimar", "instance_id": cid}, "html": "", "patches": []}))
...
def jugar_ronda(grado):
    ...
    rp = s.get(BASE + "/estimar")
    assert rp.status_code == 200, f"GET /estimar -> {rp.status_code}"
    assert 'est-eq' in rp.text, "el SSR no trae la cuenta"
    ...
    sid = psql(f"SELECT id FROM sessions WHERE profile_id={pid} AND mode='estimar' AND topic_code='estimar' ORDER BY id DESC LIMIT 1")
    assert sid, "no se creó la sesión de Estimación (estimar/estimar)"
    ...
    rec = psql(f"SELECT best_streak FROM profile_game_stats WHERE profile_id={pid} AND game_code='estimar'")
    ...
    assert n_ok == LIMIT, f"grade {grado}: estimé todo pero solo {n_ok} quedaron correctos"
    return n_ok
...
def escenario():
    ...
    assert 'href="/estimar"' in r.text, "no aparece la carta Estimación"
    # Grado 3 (redondeo) y grado 13 (estimar productos/porcentajes, nivel 5).
    g3 = jugar_ronda(3)
    g13 = jugar_ronda(13)
    return {"aparece": True, "g3_ok": g3, "g13_ok": g13}
...
if __name__ == "__main__":
    ...
    for k, v in results.items():
        print(f"  {k}: {v}")
    print("\nOK: Estimación E2E verde." if results else "nada que correr")
