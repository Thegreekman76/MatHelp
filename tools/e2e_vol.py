#!/usr/bin/env python3
# tools/e2e_vol.py — E2E del juego "Volumen" (cuerpos 3D, F9).
#
# Escenario (contra `fitz run` o el binario):
#   1. Perfil -> GET /juegos muestra "Volumen" (href="/volumen").
#   2. GET /volumen -> el SSR trae el cuerpo (SVG .vol-fig) con data-*.
#   3. Abre WS /live/volumen; en cada turno LEE kind/a/b/c/d, APLICA la fórmula del
#      volumen (cubo a³ / prisma a·b·c / cilindro 3·r²·h / compuesto a·b·c + d³) y lo
#      tipea.
#   4. Verifica: data-fb-seq, q-racha, q-progress, estrellas, ronda perfecta +
#      confetti + récord; y en Postgres: sesión mode='vol' topic 'vol', 10 attempts
#      TODOS correctos, best_streak=10 en profile_game_stats.
#
# Requiere: requests, websocket-client, psql en PATH (o FITZ_PSQL).
# Uso:  python tools/e2e_vol.py --only build   (o --only run, o ambos)
...
def resolver(html):
    m = re.search(r'vol-fig" data-kind="(\w+)" data-a="(\d+)" data-b="(\d+)" data-c="(\d+)" data-d="(\d+)"', html)
    assert m, "no llegó el cuerpo (.vol-fig con data-*)"
    kind = m.group(1)
...
def open_ws(s):
    ...
    ws = websocket.create_connection(BASE.replace("http", "ws") + "/live/volumen", header=[f"Cookie: {cookies}"], timeout=10)
...
def answer(ws, cid, ei, valor):
    ws.send(json.dumps({"event": "digito", "payload": {"d": "1", "component_name": "volumen", "instance_id": cid}, "html": "", "patches": []}))
    ws.recv()
    ws.send(json.dumps({"event": "answer", "payload": {"v": str(valor), "ei": str(ei), "touched": "1", "component_name": "volumen", "instance_id": cid}, "html": "", "patches": []}))
...
def jugar_ronda(grado):
    ...
    rp = s.get(BASE + "/volumen")
    assert rp.status_code == 200, f"GET /volumen -> {rp.status_code}"
    assert 'vol-fig' in rp.text, "el SSR no trae el cuerpo"
    ...
    sid = psql(f"SELECT id FROM sessions WHERE profile_id={pid} AND mode='vol' AND topic_code='vol' ORDER BY id DESC LIMIT 1")
    assert sid, "no se creó la sesión de Volumen (vol/vol)"
    ...
    rec = psql(f"SELECT best_streak FROM profile_game_stats WHERE profile_id={pid} AND game_code='volumen'")
...
def escenario():
    ...
    assert 'href="/volumen"' in r.text, "no aparece la carta Volumen"
    # Grado 6 (cubo/prisma) y grado 13 (cilindro/compuesto, nivel 5).
    g6 = jugar_ronda(6)
    g13 = jugar_ronda(13)
...
if __name__ == "__main__":
    ...
    for k, v in results.items():
        print(f"  {k}: {v}")
    print("\nOK: Volumen E2E verde." if results else "nada que correr")
