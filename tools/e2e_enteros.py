#!/usr/bin/env python3
# tools/e2e_enteros.py — E2E del juego "Enteros" (operaciones con signo, F9).
#
# Escenario (contra `fitz run` o el binario):
#   1. Perfil -> GET /juegos muestra "Enteros" (href="/enteros").
#   2. GET /enteros -> el SSR trae la cuenta (.ent-eq) + la recta numérica.
#   3. Abre WS /live/enteros; en cada turno LEE los operandos del data-* de la
#      cuenta, APLICA la operación con signo, y tipea el resultado (posiblemente
#      negativo, con la tecla ±).
#   4. Verifica: data-fb-seq, q-racha, q-progress, estrellas, ronda perfecta +
#      confetti + récord; y en Postgres: sesión mode='enteros' topic 'enteros', 10
#      attempts TODOS correctos, best_streak=10 en profile_game_stats.
#
# Requiere: requests, websocket-client, psql en PATH (o FITZ_PSQL).
# Uso:  python tools/e2e_enteros.py --only build   (o --only run, o ambos)
...
def resolver(html):
    m = re.search(r'ent-eq[^"]*" data-a="(-?\d+)" data-op="([^"]+)" data-b="(-?\d+)"', html)
    assert m, "no llegó la cuenta (.ent-eq con data-*)"
...
def open_ws(s):
    ...
    ws = websocket.create_connection(BASE.replace("http", "ws") + "/live/enteros", header=[f"Cookie: {cookies}"], timeout=10)
...
def answer(ws, cid, ei, valor):
    # un dígito para marcar `touched`; el valor real (posiblemente negativo) va en `v`.
    ws.send(json.dumps({"event": "digito", "payload": {"d": "1", "component_name": "enteros", "instance_id": cid}, "html": "", "patches": []}))
    ws.recv()
    ws.send(json.dumps({"event": "answer", "payload": {"v": str(valor), "ei": str(ei), "touched": "1", "component_name": "enteros", "instance_id": cid}, "html": "", "patches": []}))
...
def jugar_ronda(grado):
    ...
    rp = s.get(BASE + "/enteros")
    assert rp.status_code == 200, f"GET /enteros -> {rp.status_code}"
    assert 'ent-eq' in rp.text, "el SSR no trae la cuenta"
    ...
    sid = psql(f"SELECT id FROM sessions WHERE profile_id={pid} AND mode='enteros' AND topic_code='enteros' ORDER BY id DESC LIMIT 1")
    assert sid, "no se creó la sesión de Enteros (enteros/enteros)"
    ...
    rec = psql(f"SELECT best_streak FROM profile_game_stats WHERE profile_id={pid} AND game_code='enteros'")
...
def escenario():
    ...
    assert 'href="/enteros"' in r.text, "no aparece la carta Enteros"
    # Grado 7 (sumas con signo) y grado 13 (multiplicación de signos, nivel 5).
    g7 = jugar_ronda(7)
    g13 = jugar_ronda(13)
    return {"aparece": True, "g7_ok": g7, "g13_ok": g13}
...
if __name__ == "__main__":
    ...
    for k, v in results.items():
        print(f"  {k}: {v}")
    print("\nOK: Enteros E2E verde." if results else "nada que correr")
